...
def fetch_url(request):
    if request.method == 'POST':
        try:
            ...
            url = data.get('url')
            if not url:
                return JsonResponse({'success': False, 'error': '無效的網址'})
            
            response = requests.get(url)
            response.raise_for_status()  # 檢查請求是否成功
            content = response.content.decode('utf-8')  # 轉為字串以便返回
            
            return JsonResponse({'success': True, 'content': content})
        except requests.exceptions.RequestException as e:
            return JsonResponse({'success': False, 'error': str(e)})
    return JsonResponse({'success': False, 'error': '只接受 POST 請求'})
...
def compare_code_raw(request):
    if request.method == 'POST':
        code_input1 = request.POST.get('code_input1', '')
        code_input2 = request.POST.get('code_input2', '')

        matcher = SequenceMatcher(None, code_input1, code_input2)
        result_left = ""
        result_right = ""

        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            part1 = code_input1[i1:i2]
            part2 = code_input2[j1:j2]

            if tag == 'equal':
                # 将相同的部分标记为绿色
                result_left += f'<span class="highlight">{part1.replace("<", "&lt;").replace(">", "&gt;")}</span>'
                result_right += part2.replace("<", "&lt;").replace(">", "&gt;")
            else:
                # 非匹配部分保持原样
                result_left += part1.replace("<", "&lt;").replace(">", "&gt;")
                result_right += f'<span class="non-matching">{part2.replace("<", "&lt;").replace(">", "&gt;")}</span>'

        return JsonResponse({'result_left': result_left, 'result_right': result_right})
    else:
        return JsonResponse({'error': 'Invalid request method.'}, status=400)